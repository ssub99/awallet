 ...
 * Android: ADJUST_NOTHING(탭바 고정) + keyboard-controller height를
 *          useAnimatedStyle에서 직접 읽어 키보드와 동일 프레임으로 이동.
 *          JS DidShow/metrics/catch-up 보정 레이어는 쓰지 않음.
 */
 ...
 * 오버레이 위치는 keyboard-controller height SharedValue로 맞춤.
 */
...
/** 간편입력 수입 기록 바텀시트 analytics target (Amplitude 설계 151~161행) */
const INCOME_RECORD_SHEET_ANALYTICS_TARGETS = new Set([
  'cta',
  'category',
  'category-option',
  'category-option-confirm',
  'category-option-prev',
  'calendar',
  'calendar-confirm',
  'calendar-close',
  'amount',
...
function logExpenseRecordSheetEvent(
  ...
  screenNameOverride?: string | null,
): void {
  const screenName =
    screenNameOverride ??
    (recordType === 'income'
  ...
  if (recordType === 'income' && !INCOME_RECORD_SHEET_ANALYTICS_TARGETS.has(target)) {
    return;
  }
  if (__DEV__) {
...
export const QuickInputProvider = ({ children }: PropsWithChildren) => {
  ...
  const editAmountKeypadHeight = getKeypadHeight(windowWidth);
  ...
  const [quickInputEditRecordType, setQuickInputEditRecordType] = useState<'expense' | 'income'>('expense');
  ...
  const logRecordSheetEvent = useCallback(
    (eventName: string, target: string, extra?: Record<string, unknown>) => {
      logExpenseRecordSheetEvent(
        pendingRecordRef.current?.recordType,
        eventName,
        target,
        extra,
  ...
  /** 계산기/시트 등으로 follow를 끌 때 visible 플래그만 리셋 */
  ...
  /**
   * Android: keyboard-controller height SharedValue를 그대로 bottom에 사용.
   * (KeyboardStickyView와 동일 — 키보드와 같은 프레임으로 움직임)
   * iOS: animatedBottom (handler follow).
   */
  ...
  }, [overlayStarScale, overlayStarRotate, setShouldFollowKeyboard]);
  ...
  /** 설정 퍼널과 동일: list로 돌아갈 때는 draft를 건드리지 않음. 생성 진입·시트 닫을 때만 리셋. */
  const resetQuickInputCategoryDraft = useCallback(() => {
    setQuickInputCategoryDraftName('');
    setQuickInputCategoryDraftEmoji('✅');
    setQuickInputCategoryOriginal(null);
  }, []);
  ...
  const handleConfirmCardChange = useCallback(() => {
    ...
    const editCategoryLabel = pending.category ?? '';
    const editCategoryEmoji =
      confirmCardData?.categoryEmoji ??
      (editCategoryType === 'income'
        ? incomeCategoriesCacheRef.current
        : expenseCategoriesCacheRef.current
      ).find((category) => category.label === editCategoryLabel)?.emoji ??
      getCategoriesByType(editCategoryType).find((category) => category.label === editCategoryLabel)?.emoji;
    setQuickInputEditDraft({
      category: editCategoryLabel,
      categoryEmoji: editCategoryEmoji,
      date: pending.date.replace(/-/g, '.'),
  ...
  const quickInputEditCategoryEmoji = useMemo(() => {
    if (quickInputEditDraft?.categoryEmoji) {
      return quickInputEditDraft.categoryEmoji;
    }
    ...
    const cachedList =
      quickInputEditRecordType === 'income'
        ? incomeCategoriesCacheRef.current
        : expenseCategoriesCacheRef.current;
    return (
      cachedList.find((category) => category.label === categoryLabel)?.emoji ??
      quickInputCategorySheetCategories.find((category) => category.label === categoryLabel)?.emoji ??
      getCategoriesByType(quickInputEditRecordType).find((category) => category.label === categoryLabel)?.emoji
    );
  }, [
    quickInputEditDraft?.category,
    quickInputEditDraft?.categoryEmoji,
    quickInputEditRecordType,
    quickInputCategorySheetCategories,
  ]);
  ...
  const quickInputEditSheetTitle =
    quickInputEditRecordType === 'income' ? '수입 기록' : '소비 기록';
  const quickInputEditDateSheetTitle =
    quickInputEditRecordType === 'income' ? '수입 기록일 선택' : '소비 기록일 선택';
  const isQuickInputEditIncome = quickInputEditRecordType === 'income';
  ...
  const handleQuickInputEditCategoryPress = useCallback(() => {
    logRecordSheetEvent('ui', 'category');
  ...
  const handleQuickInputCategorySheetClose = useCallback(() => {
    logRecordSheetEvent('btn', 'category-option-prev');
    setQuickInputEditView('form');
  }, []);
  ...
  const handleQuickInputCategorySheetConfirm = useCallback(() => {
    ...
    logRecordSheetEvent('btn', 'category-option-confirm', {
      category: quickInputCategorySheetSelected,
    });
    const selectedCategory = quickInputCategorySheetCategories.find(
      (category) => category.label === quickInputCategorySheetSelected,
    );
    updateQuickInputEditDraft({
      category: quickInputCategorySheetSelected,
  ...
  const handleQuickInputCategoryOptionPress = useCallback((category: Category) => {
    logRecordSheetEvent('list', 'category-option', {
      category: category.label,
    });
  ...
  const handleQuickInputEditDatePress = useCallback(() => {
    logRecordSheetEvent('ui', 'calendar');
    closeQuickInputEditAmountKeypad({ immediate: true });
    ...
    logRecordSheetEvent('sheet_view', 'calendar');
  ...
  const handleQuickInputEditPaymentPress = useCallback(() => {
    if (pendingRecordRef.current?.recordType === 'income') {
      return;
    }
    logRecordSheetEvent('ui', 'payment');
    ...
    logRecordSheetEvent('sheet_view', 'payment');
  ...
  const handleQuickInputPaymentSheetClose = useCallback(() => {
    logRecordSheetEvent('btn', 'payment-close');
  ...
  const handleQuickInputPaymentSelect = useCallback((method: 'credit' | 'debit' | 'cash', subtype?: PaymentSubtype) => {
    if (method === 'cash') {
      logRecordSheetEvent('btn', 'payment-cash');
    } else {
      logRecordSheetEvent('list', method === 'credit' ? 'payment-credit' : 'payment-debit');
    }
    updateQuickInputEditDraft({
  ...
  const handleQuickInputEditRecurringPress = useCallback(() => {
    ...
    logRecordSheetEvent('btn', 'option');
    closeQuickInputEditAmountKeypad({ immediate: true });
    ...
    logRecordSheetEvent('sheet_view', 'recurring-installment-sheet');
  }, [closeQuickInputEditAmountKeypad, quickInputEditDraft]);

  const handleQuickInputRecurringSheetClose = useCallback(() => {
    logRecordSheetEvent('btn', 'recurring-installment-sheet-close');
    setQuickInputEditView('form');
  }, []);

  const handleQuickInputRecurringConfirm = useCallback(() => {
    logRecordSheetEvent('btn', 'recurring-installment-sheet-confirm');
    updateQuickInputEditDraft({
  ...
  const handleQuickInputRecurringToggle = useCallback((value: boolean) => {
    logRecordSheetEvent('ui', 'recurring-toggle');
  ...
  const handleQuickInputInstallmentToggle = useCallback((value: boolean) => {
    logRecordSheetEvent('ui', 'installment-toggle');
  ...
  const handleQuickInputEditSheetClose = useCallback((options?: { fromConfirm?: boolean }) => {
  ...
  const handleQuickInputEditSheetConfirm = useCallback(() => {
    ...
    logRecordSheetEvent('btn', 'cta');

    const isIncomeEdit = current.recordType === 'income';
    const updated: PendingParseRecord = isIncomeEdit
      ? {
          ...current,
  ...
  /**
   * Android: 위치는 keyboardReanimated.height. JS는 dismiss/숨김만.
   */
  useEffect(() => {
    if (Platform.OS !== 'android' || !isQuickInputVisible) {
      return;
    ...
    return () => onHide.remove();
  }, [finishHideQuickInput, handleAndroidKeyboardDismissed, isQuickInputVisible]);
        ...
        {isQuickInputVisible && !isQuickInputOverlaySuppressed && (
          <View style={styles.overlay} pointerEvents="box-none">
                    ...
                    {!quickInputEditSheetVisible && !isQuickInputEditSheetClosing && isQuickInputCalculatorMounted && (
                      <RNAnimated.View
                        style={[
                          ...
                          {
                            paddingBottom: androidKeypadSafeBottom,
              ...
              <ModalBottomsheet
                ...
                title={quickInputEditSheetTitle}
                    ...
                    <BottomSheetFlow
                      ...
                      screens={[
                        ...
                        },
                        {
                          ...
                          swipeBackEnabled: true,
                          content: (
                            <View style={styles.recurringSheetRoot}>
                              <ScrollView
                                      ...
                                      {(quickInputRecurringDraftIsPeriodExpanded ? QUICK_INPUT_RECURRING_PERIOD_OPTIONS : QUICK_INPUT_RECURRING_PERIOD_OPTIONS.slice(0, 6)).map((label) => (
                                        <Chip
                                          ...
                                          onPress={() => {
                                            ...
                                            if (recurringTarget) {
                                              logRecordSheetEvent('ui', recurringTarget);
                                            }
                                        ...
                                        <Chip
                                          ...
                                          onPress={() => {
                                            ...
                                            if (installmentTarget) {
                                              logRecordSheetEvent('ui', installmentTarget);
                                            }
                                      ...
                                      <View key={option.value}>
                                        <Pressable
                                          ...
                                          onPress={() => {
                                            ...
                                            logRecordSheetEvent('ui', QUICK_INPUT_WEEKEND_ANALYTICS_TARGET[option.value]);
                              ...
                              <View style={styles.recurringSheetBottomInsetWrap}>
                                <ModalBottomsheetBottomInset backgroundColor={atomicColors.neutral[100]} />
                              </View>
                            </View>
                          ),
              ...
              {quickInputDateSheetMounted ? (
                <RecordDatePickerSheet
                  ...
                  embedded
                  title={quickInputEditDateSheetTitle}
