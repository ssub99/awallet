...
class SmsInboxReceiver : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent) {
    ...
    if (sender.isEmpty() || !SmsInboxNativeStore.isSenderAllowed(context, sender)) return

    val body = messages.joinToString(separator = "") { it.messageBody.orEmpty() }.trim()
    if (body.isEmpty() || !SmsInboxNativeStore.hasSupportedTransactionKeyword(body)) return
